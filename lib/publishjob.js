...
PublishJob.prototype.generateTocData = function(options) {
    ...
    function addItems(data, n) {
        Object.keys(data).sort().forEach(function(key) {
            //var str = new Array(n).fill("\t").join("");
            //console.log(str + "Adding: key " + key);// + ", value \n" + JSON.stringify(data[key]) + "\n");
            var item = data[key];
            var tocEntry;
    ...
    _.each(navTree, function(item, key) {
        ...
        console.log("item.doclet is " + item.doclet);
        if (item.doclet) console.log("item.doclet.kind is " + item.doclet.kind);*/
        if (item.doclet && item.doclet.kind) {
            var kind = item.doclet.kind;
            //console.log("kind is " + kind);
            docletKinds[kind] = docletKinds[kind] || {};
            docletKinds[kind][key].children = item.children;
        }
    });

    _.each(navTree, function(item, key) {
        if (item.doclet && item.doclet.kind === 'external') {
            externalsItems[key] = item;
        } else if (key.startsWith('module:')) {
            console.log("Adding module " + key);
            docletKinds['module'].children.push(item);
        } else {
            var children = docletKinds['class'].children;
            console.log("Adding class " + key);
            children.push(item);
        }
    });

    _.each(navTreeItems, function(item, key) {
        console.log("key is " + key);
        console.log("item is ", item);
    });


    addItems(docletKinds);
